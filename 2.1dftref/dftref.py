"""
Single-point UKS reference calculation for the full Al50+O2 system at d=2.1 A.
...
newton=False)`` in ``fragments.py``):
    ...
    - direct_scf = True, max_cycle = 200, conv_tol = 1e-6
...
The full-system molecule mirrors the geo built in ``2.1capdft.py``:
``atom='2.xyz'``, ``basis={'O': obasis, 'Al': albasis}``, ``charge=0``,
``spin=2``.
...
For sigma = 0.002 we:
    ...
    3. Dump:
        - ``al2.1_sigma{sigma}.chk`` via pyscf.scf.chkfile.dump_scf (standard
          chk file, loadable with scfchk.load_scf).
        - ``al2.1_sigma{sigma}_last_dm.pkl`` with the last-normal-cycle
...
def run_one_sigma(sigma):
    ...
    chkname = f"al2.1_sigma{sigma:g}.chk"
    scfchk.dump_scf(
        mol, chkname,
        e_tot=e_tot,
    ...
    pklname = f"al2.1_sigma{sigma:g}_last_dm.pkl"
