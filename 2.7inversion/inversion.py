...
# n2v only assigns T_pbs when pbs != "same"; Wu–Yang regularization still needs it.
_orig_set_basis_matrices = _n2v_inverter.Inverter.set_basis_matrices


def _set_basis_matrices_with_tpbs_same(self):
    _orig_set_basis_matrices(self)
    if getattr(self.eng, "pbs_str", None) == "same":
        self.T_pbs = self.T


_n2v_inverter.Inverter.set_basis_matrices = _set_basis_matrices_with_tpbs_same
...
# Paths (match 2.7pdft/2.7capdft.py: latest PDFT + smeared d=2.7 dftref, sigma=0.002)
...
DFTREF_DM_PKL = os.path.join(_DIR, "..", "2.7dftref", "al2.7_sigma0.002_last_dm.pkl")
...
def _resolve_mol_xyz(mol, chk_path):
    """Chk files store mol.atom as a bare filename (e.g. 4.xyz); resolve via dftref dir."""
    atom = mol.atom
...
lgeo1.atom= "o.xyz"
lgeo1.unit='angstrom'
# Match 2.7capdft.py / pdft2.7_checkpointnewb.pkl (ghost-Al basis on lgeo1).
...
# ref=2 (UKS): inverter solves separate v_pbs for alpha and beta channels
# (length 2*npbs). At d=2.7 the orbital gaps shrink (near-dissociation),
# so we use BFGS + warm-start from a prior inversion pickle
# rather than trust-krylov, which is fragile here.
inv.set_system(mol, _inv_basis, pbs="6-311g**", ref=2)
inv.Dt = Dtotnew
# Align integer occupations with projected target density (avoids molecule vs fragment mismatch).
S2_inv = inv.S2
