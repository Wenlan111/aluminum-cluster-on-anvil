...
PDFT_CHECKPOINT = os.path.join(_DIR, "..", "2.4pdft", "pdft_checkpointnewref6.pkl")
DFTREF_CHK = os.path.join(_DIR, "..", "2.4dftref", "al2.4_sigma0.002.chk")
...
#projected densitys
ao_values_rnew = dft.numint.eval_ao(rgeonew, coords, deriv=1)
ao_values_lnew = dft.numint.eval_ao(lgeonew, coords, deriv=1)
rho_r_a_proj = dft.numint.eval_rho(rgeonew, ao_values_rnew, Darnew, xctype="GGA")
rho_r_b_proj = dft.numint.eval_rho(rgeonew, ao_values_rnew, Dbrnew, xctype="GGA")
rho_l_a_proj = dft.numint.eval_rho(lgeonew, ao_values_lnew, Dalnew, xctype="GGA")
rho_l_b_proj = dft.numint.eval_rho(lgeonew, ao_values_lnew, Dblnew, xctype="GGA")
rho_r_proj = rho_r_a_proj + rho_r_b_proj
rho_l_proj = rho_l_a_proj + rho_l_b_proj
rho_tot_a_proj = rho_r_a_proj + rho_l_a_proj
rho_tot_b_proj = rho_r_b_proj + rho_l_b_proj
rho_tot_proj = rho_tot_a_proj + rho_tot_b_proj

#evaluate projected exc
xc = "PBE"
exc_proj = dft.libxc.eval_xc(xc, (rho_tot_a_proj, rho_tot_b_proj), spin=1)[0]
Exc_proj = float(np.dot(w, exc_proj * rho_tot_proj[0]))
print("Exc_proj =", Exc_proj)
excl_proj = dft.libxc.eval_xc(xc, (rho_l_a_proj, rho_l_b_proj), spin=1)[0]
excr_proj = dft.libxc.eval_xc(xc, (rho_r_a_proj, rho_r_b_proj), spin=1)[0]
Excl_proj = float(np.dot(w, excl_proj * rho_l_proj[0]))
Excr_proj = float(np.dot(w, excr_proj * rho_r_proj[0]))
print("Excl_proj =", Excl_proj, "Excr_proj =", Excr_proj)

print("Excna_proj =", Exc_proj - Excl_proj - Excr_proj)
