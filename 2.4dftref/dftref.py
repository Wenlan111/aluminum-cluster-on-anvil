"""
Single-point UKS reference calculation for the full Al50+O2 system at d=2.4 A.
...
The full-system molecule mirrors the geo built in ``2.4capdft.py``:
``atom='3.xyz'``, ``basis={'O': obasis, 'Al': albasis}``, ``charge=0``,
``spin=2``.
...
For each sigma in [0.002, 0.005, 0.01, 0.02] we:
    ...
    3. Dump:
        - ``al2.4_sigma{sigma}.chk`` via pyscf.scf.chkfile.dump_scf (standard
          chk file, loadable with scfchk.load_scf).
        - ``al2.4_sigma{sigma}_last_dm.pkl`` with the last-normal-cycle
...
def run_one_sigma(sigma):
    ...
    chkname = f"al2.4_sigma{sigma:g}.chk"
    scfchk.dump_scf(
        mol, chkname,
        e_tot=e_tot,
    ...
    pklname = f"al2.4_sigma{sigma:g}_last_dm.pkl"
